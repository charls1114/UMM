			...
			try {
				...
				do
				{
					do
					{
						...
						vector<IZoomVideoSDKUser*> userlist = ZoomVideoSDKMgr::GetInst().GetAllUsers();

						for (auto user : userlist)
						{
							OutputDebugString(user->getUserName());
							ZoomVideoSDKMgr::GetInst().SendChatToUser(user, stt_text);
						}
